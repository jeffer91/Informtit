...
replace_once("const VERSION = '4.1.0';", "const VERSION = '4.2.0';", 'versión 4.1')
...
"""  function periodKey(value = '') {
  ...
  function normalizeCampus(value = '') {
    const campus = fold(value).replace(/\\b(SEDE|CAMPUS)\\b/g, ' ').replace(/\\s+/g, ' ').trim();
    if (campus === 'SUR') return 'SUR';
    if (campus === 'MANTA') return 'MANTA';
    if (campus === 'MATRIZ') return 'MATRIZ';
    return campus;
  }
...
new_complexive = r'''  function reconcileComplexive(rawRows, master) {
    ...
    const parsed = rawRows.map((raw, index) => {
      ...
      return {
        index:index+2, cedula, studentId, nombre:sourceName, carrera:sourceCareer,
        modalidad:normalizeModality(sourceModality, matched?.enrollment?.career_code || ''),
        sede:sourceCampus, periodo:sourcePeriod, tipo:clean(pick(row, 'tipo_titulacion')),
        teorico, practico, acumulado, supletorio, calculado, final, estadoFinal,
        matched, warnings, errors, raw,
      };
    });
...
'''
text = text[:start] + new_complexive + text[end:]

start = text.index('  function reconcileNuclei(rawRows, master) {')
end = text.index('\n  function stats(rows) {', start)
new_nuclei = r'''  function reconcileNuclei(rawRows, master) {
    const seen = new Set();
    ...
    const parsed = rawRows.map((raw, index) => {
      ...
      return {
        index:index+2, cedula, studentId, nucleus, nombre:sourceName, carrera:sourceCareer,
        ...
        materia:clean(pick(row, 'materia')), docente:clean(pick(row, 'Pro_nombre')),
        nota, resultado, calculated:false, matched, warnings, errors, raw,
      };
    });
    ...
    byStudent.forEach(items => {
      ...
      const isThesis = studentId && master.thesisIds.has(studentId);
      const hasComplexive = studentId && master.complexiveIds.has(studentId);
...
replace_once(
"{ accumulated_grade_structured:true }",
"{ accumulated_grade_structured:true, incidents:rows.filter(item=>item.errors.length).map(item=>({cedula:item.cedula,fila:item.index,motivos:item.errors})).slice(0,500) }",
'incidencias Complexivo')

replace_once(
"{ matching:'CEDULA', tt_priority:true, four_nuclei_validated:true }",
"{ matching:'CEDULA', tt_priority:true, four_nuclei_validated:true, calculated_nuclei:valid.filter(item=>item.calculated).length, incidents:rows.filter(item=>item.errors.length).map(item=>({cedula:item.cedula,fila:item.index,motivos:item.errors})).slice(0,500) }",
'incidencias Núcleos')

# Exponer helpers para pruebas y auditoría.
replace_once(
"""    buildNucleusCatalog,
    nucleusNumber,
  });
