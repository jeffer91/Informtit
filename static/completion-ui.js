(() => {
  ...
  function debounceRender() {
    ...
    setTimeout(() => {
      if (token !== renderToken || !state.activeReport?.id) return;
      enhanceSchedules();
      renderEligibility();
  ...
  renderReport = function renderReportWithCompletion() {
  ...
  function decorateScheduleTable(card, items) {
    ...
    if (!headerRow.querySelector('[data-execution-header]')) {
      ...
      actionHeader.insertAdjacentHTML('beforebegin', `
    ...
    [...table.querySelectorAll('tbody tr')].forEach((row, index) => {
      if (row.querySelector('[name="executed_date"]')) return;
  ...
  async function enhanceSchedules() {
    ...
    if (!reportId || !tab || !tab.querySelector('[data-schedule-card]')) return;
    try {
      ...
      for (const type of ['complexive', 'thesis']) {
        ...
        if (!card) continue;
        decorateScheduleTable(card, data.schedules[type] || []);
      ...
      // La interfaz original de cronogramas sigue disponible si falla la extensión.
    }
  }
  ...
  function eligibilityMarkup(data, reportId) {
    const summary = data.summary;
    return `<section class="panel eligibility-panel" data-eligibility-panel="${reportId}">
      <div class="panel-head">
        <div>
          <h2>Habilitación para el Examen Complexivo</h2>
          <p>Cada estudiante debe aprobar los cuatro núcleos con una calificación mínima de 7,00. Los núcleos no se compensan entre sí.</p>
        </div>
      </div>
      ...
      <div class="student-table-wrap"><table class="student-table compact-table">
        <thead><tr><th>Carrera</th><th>Candidatos</th><th>Habilitados</th><th>No habilitados</th><th>Pendientes</th><th>% habilitación</th></tr></thead>
        <tbody>${data.careers.map(row => `<tr><td>${esc(row.career_name)}</td><td>${row.total}</td><td>${row.habilitated}</td><td>${row.not_habilitated}</td><td>${row.pending}</td><td>${fmt(row.habilitation_percentage)} %</td></tr>`).join('')}</tbody>
      </table></div>
  ...
  async function renderEligibility() {
    ...
    if (!reportId || !tab || !tab.querySelector('.process-stack')) return;
    try {
      const data = await api(`/api/reports/${reportId}/nuclei/eligibility`);
      if (Number(state.activeReport?.id || 0) !== reportId) return;
      tab.querySelector('[data-eligibility-panel]')?.remove();
      tab.querySelector('.process-stack').insertAdjacentHTML('beforeend', eligibilityMarkup(data, reportId));
      const search = tab.querySelector('[data-eligibility-search]');
      search?.addEventListener('input', () => {
        const query = search.value.trim().toLowerCase();
        tab.querySelectorAll('.eligibility-table tbody tr').forEach(row => {
          row.hidden = query && !row.dataset.searchValue.includes(query);
        });
      });
      renderComplexiveEligibilityWarning(data, reportId);
    } catch (_error) {
  ...
  function renderComplexiveEligibilityWarning(data, reportId) {
    const tab = document.querySelector('#tab-careers');
    if (!tab) return;
    tab.querySelector('[data-complexive-eligibility-warning]')?.remove();
  ...
  function incidentRow(item = {}) {
    return `<tr>
      <td><input class="table-input" name="category" value="${esc(item.category || '')}" placeholder="Notas, requisitos, plataforma..."></td>
      <td><textarea class="table-input" name="description" rows="2" placeholder="Descripción de la novedad">${esc(item.description || '')}</textarea></td>
      <td><input class="table-input" name="responsible" value="${esc(item.responsible || '')}" placeholder="Unidad responsable"></td>
      <td><textarea class="table-input" name="treatment" rows="2" placeholder="Tratamiento aplicado">${esc(item.treatment || '')}</textarea></td>
      <td><select class="table-input" name="status">${['Abierto', 'En seguimiento', 'Resuelto'].map(status => `<option ${status === (item.status || 'Abierto') ? 'selected' : ''}>${status}</option>`).join('')}</select></td>
      <td><input class="table-input" name="evidence" value="${esc(item.evidence || '')}" placeholder="Evidencia"></td>
      <td><button type="button" class="button danger small" data-remove-completion-row>Eliminar</button></td>
    </tr>`;
  }

  function actionRow(item = {}) {
    return `<tr>
      <td><textarea class="table-input" name="finding" rows="2" placeholder="Hallazgo">${esc(item.finding || '')}</textarea></td>
      <td><textarea class="table-input" name="action" rows="2" placeholder="Acción de mejora">${esc(item.action || '')}</textarea></td>
      <td><input class="table-input" name="responsible" value="${esc(item.responsible || '')}" placeholder="Responsable"></td>
      <td><input class="table-input date-input" name="due_date" value="${esc(item.due_date || '')}" placeholder="dd/mm/aaaa"></td>
      <td><input class="table-input" name="indicator" value="${esc(item.indicator || '')}" placeholder="Indicador"></td>
      <td><input class="table-input" name="evidence" value="${esc(item.evidence || '')}" placeholder="Evidencia"></td>
      <td><select class="table-input" name="status">${['Pendiente', 'En ejecución', 'Cumplida'].map(status => `<option ${status === (item.status || 'Pendiente') ? 'selected' : ''}>${status}</option>`).join('')}</select></td>
      <td><button type="button" class="button danger small" data-remove-completion-row>Eliminar</button></td>
    </tr>`;
  }

  function completionMarkup(data, reportId) {
    return `<section class="panel completion-panel" data-completion-panel="${reportId}">
      <div class="panel-head"><div><h2>Novedades, incidencias y plan de mejora</h2><p>Registre únicamente situaciones verificadas. Estos datos se incorporarán al informe final.</p></div><button class="button primary small" data-save-completion>Guardar seguimiento</button></div>
      <h3>Novedades e incidencias</h3>
      <div class="student-table-wrap"><table class="student-table completion-table" data-incident-table><thead><tr><th>Categoría</th><th>Descripción</th><th>Responsable</th><th>Tratamiento</th><th>Estado</th><th>Evidencia</th><th></th></tr></thead><tbody>${data.incidents.map(incidentRow).join('')}</tbody></table></div>
      <button type="button" class="button secondary small" data-add-incident>Agregar incidencia</button>
      <h3>Plan de mejora</h3>
      <div class="student-table-wrap"><table class="student-table completion-table" data-action-table><thead><tr><th>Hallazgo</th><th>Acción</th><th>Responsable</th><th>Fecha límite</th><th>Indicador</th><th>Evidencia</th><th>Estado</th><th></th></tr></thead><tbody>${data.actions.map(actionRow).join('')}</tbody></table></div>
      <button type="button" class="button secondary small" data-add-action>Agregar acción</button>
    </section>`;
  }
  ...
  async function renderCompletionPanel() {
    ...
    try {
      ...
      panel.addEventListener('click', async event => {
        if (event.target.closest('[data-remove-completion-row]')) {
          event.target.closest('tr').remove();
          return;
        }
        if (event.target.closest('[data-add-incident]')) {
          panel.querySelector('[data-incident-table] tbody').insertAdjacentHTML('beforeend', incidentRow());
          return;
        }
        if (event.target.closest('[data-add-action]')) {
          panel.querySelector('[data-action-table] tbody').insertAdjacentHTML('beforeend', actionRow());
          return;
        }
        if (event.target.closest('[data-save-completion]')) {
          try {
            ...
            const result = await api(`/api/reports/${reportId}/completion`, {
              method: 'PUT',
              body: JSON.stringify({ incidents, actions }),
            });
      ...
      // Los cronogramas permanecen operativos aunque falle el seguimiento.
    }
  }
  ...
  new MutationObserver(debounceRender).observe(document.body, { childList: true, subtree: true });
  document.addEventListener('click', event => {
    if (event.target.closest('[data-tab="schedules"], [data-tab="nuclei"], [data-tab="careers"], [data-save-nucleus]')) {
      debounceRender();
    }
  });
  debounceRender();
