(() => {
  ...
  function footer(text='Neon PostgreSQL') {
    const node=document.querySelector('.sidebar-footer span:last-child');
    if(node&&node.textContent!==text)node.textContent=text;
  }
  function sourceBadge(ok=true) {
    let node=document.getElementById('neon-source-state');
    if(!node){node=document.createElement('div');node.id='neon-source-state';node.style.cssText='margin:0 0 12px;padding:9px 11px;border:1px solid #cfe4d7;border-radius:10px;background:#f2fbf5;color:#285d3d;font-size:12px';const dashboard=document.getElementById('view-dashboard');dashboard?.insertBefore(node,dashboard.firstChild);}
    node.textContent=ok?'Neon PostgreSQL conectado · fuente principal':'Neon no respondió · revise la sesión o conexión';
    node.style.background=ok?'#f2fbf5':'#fff4f2';node.style.borderColor=ok?'#cfe4d7':'#efc7c1';node.style.color=ok?'#285d3d':'#8b342e';
  }
  async function ping(){try{const data=await window.InformtitSheets?.ping?.();const ok=Boolean(data?.ok);footer(ok?'Neon conectado':'Neon PostgreSQL');sourceBadge(ok);}catch(_){footer('Neon PostgreSQL');sourceBadge(false);}}

  const previousDashboard=window.renderDashboard;
  if(typeof previousDashboard==='function')window.renderDashboard=function(...args){const result=previousDashboard.apply(this,args);removeLegacy();const cards=[...document.querySelectorAll('#dashboard-metrics .metric')];const db=cards.find(card=>/base de datos/i.test(card.querySelector('span')?.textContent||''));if(db){const value=db.querySelector('strong');if(value)value.textContent='Neon PostgreSQL';}setTimeout(()=>void ping(),0);return result;};
