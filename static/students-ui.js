(() => {
  ...
  function isPvc() { return String(window.state?.activeReport?.report_type || '').toLowerCase() === 'pvc'; }
  function reportId() {
    const candidates = [window.state?.activeReport?.id, window.state?.activeReportId, window.activeReportId, document.body?.dataset?.reportId];
  ...
  function badge(label, kind = '') { return `<span class="student-badge ${kind}">${esc(label)}</span>`; }

  function safeRoute(row) {
    if (isPvc()) return 'ARTICULO';
    const hasThesis = Boolean(row?.has_thesis);
    const hasComplexive = Boolean(row?.has_complexive);
    const hasNuclei = Boolean(row?.has_nuclei);
    if (hasThesis && !hasComplexive && !hasNuclei) return 'TRABAJO_TITULACION';
    if (hasComplexive || hasNuclei) return 'COMPLEXIVO';
    return 'SIN_DEFINIR';
  }

  function formatMissing(row) {
    const missing = Array.isArray(row.missing_requirements) ? row.missing_requirements : Array.isArray(row.missing_requirement_labels) ? row.missing_requirement_labels : [];
    if (!missing.length) return badge('9/9 habilitantes', 'ok');
    if (missing.length === 1) return badge(`Falta: ${missing[0]}`, 'warn');
    return badge(`${missing.length} requisitos pendientes`, 'danger');
  }

  function computedSummary(students, source = {}) {
    return {
      ...source,
      students: students.length,
      complexive: students.filter(row => safeRoute(row) === 'COMPLEXIVO').length,
      thesis: students.filter(row => safeRoute(row) === 'TRABAJO_TITULACION').length,
      article: students.filter(row => safeRoute(row) === 'ARTICULO').length,
      review: students.filter(row => safeRoute(row) === 'SIN_DEFINIR').length,
    };
  }
  ...
  function renderSummary(summary) {
    return `
      <div class="student-summary-grid">
        ...
        <div class="student-metric"><strong>${summary.review || 0}</strong><span>Ruta por definir</span></div>
      </div>`;
  }

  function rowHtml(row) {
    const route = safeRoute(row);
    const routeKind = route === 'SIN_DEFINIR' ? 'warn' : 'ok';
    const reconciliation = row.reconciliation_status === 'OK' ? badge('Correcto', 'ok') : badge(row.reconciliation_status || 'Revisión', 'warn');
    const official = Number(row.official_graduated) === 1 ? badge('Graduado', 'ok') : badge('No graduado', 'muted');
    return `
      <tr data-student-id="${Number(row.id)}" data-search="${esc(`${row.identification} ${row.full_name} ${row.email} ${row.career_name}`.toLowerCase())}"
          data-route="${esc(route)}" data-process="${esc(row.process_status)}" data-reconciliation="${esc(row.reconciliation_status)}">
        ...
        <td>${badge(ROUTE_LABELS[route] || route, routeKind)}<small>${route === 'SIN_DEFINIR' ? 'Sin evidencia suficiente para asignar una ruta' : 'Derivada de la evidencia institucional cargada'}</small></td>
  ...
  function detailHtml(row) {
    ...
    return `
      <div class="student-detail-card">
        <div class="student-detail-head"><div><h3>${esc(row.full_name)}</h3><p>${esc(row.identification)} · ${esc(row.career_name)}</p></div><button class="icon-button student-detail-close" type="button">×</button></div>
        <div class="student-detail-grid">
          <div><span>Modalidad</span><strong>${esc(row.modality === 'en_linea' ? 'Online' : 'Presencial')}</strong></div>
          <div><span>Ruta</span><strong>${esc(ROUTE_LABELS[route] || route)}</strong></div>
        ...
        ${route === 'SIN_DEFINIR' ? '<div class="student-alert">La ruta no se asigna por defecto. Se requiere evidencia institucional de Complexivo o Trabajo de Titulación.</div>' : ''}
        <h4>Requisitos habilitantes pendientes</h4>
        <p>${missing.length ? esc(missing.join(', ')) : 'Ninguno. Los nueve requisitos habilitantes están completos.'}</p>
        <h4>Vínculos de fuentes</h4>
        ${links.length ? `<div class="student-links-list">${links.map(link => `<div><strong>${esc(link.source_module)}</strong><span>${esc(link.source_name || link.source_key)}</span><span>${esc(link.match_status)} · ${esc(link.match_method || '')}${link.match_confidence != null ? ` · ${esc(link.match_confidence)}%` : ''}</span></div>`).join('')}</div>` : '<p>Aún no hay fuentes conciliadas para este estudiante.</p>'}
  ...
  async function renderStudents() {
    ...
    root.innerHTML = '<div class="panel"><p>Cargando estudiantes...</p></div>';
    try {
      const data = await api(`/api/reports/${id}/students-domain`);
      const students = data.students || [];
      const summary = computedSummary(students, data.summary || {});
      root._studentsData = students;
      root.innerHTML = `
        <div class="students-domain">
          <div class="panel">
            <div class="panel-head"><div><h2>Estudiantes</h2><p>La ruta se muestra únicamente cuando existe evidencia institucional suficiente.</p></div></div>
            ${renderSummary(summary)}
            <div class="student-filters">
              <input id="student-search" placeholder="Buscar por cédula, nombre, correo o carrera">
              <select id="student-filter-route"><option value="">Todas las rutas</option><option value="COMPLEXIVO">Complexivo</option><option value="TRABAJO_TITULACION">Trabajo de Titulación</option><option value="ARTICULO">Artículo Académico</option><option value="SIN_DEFINIR">Por definir</option></select>
