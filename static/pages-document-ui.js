(() => {
  ...
  function stateKey(kind) { return `${Number(window.state?.activeReport?.id || 0)}:${kind}`; }
  function getSaveState(kind) { return saveStates.get(stateKey(kind)) || 'idle'; }
  function setSaveState(kind, value) { saveStates.set(stateKey(kind), value); }
  function buttonText(kind, defaultLabel) {
    const state = getSaveState(kind);
    if (state === 'dirty') return 'Guardar cambios';
    if (state === 'saving') return 'Guardando…';
    if (state === 'saved') return 'Guardado';
    if (state === 'local') return 'Solo local';
    if (state === 'error') return 'Error al guardar';
    return defaultLabel;
  }
  function buttonClass(kind) {
    const state = getSaveState(kind);
    if (state === 'saved') return ' is-saved';
    if (state === 'local') return ' is-warning';
    if (state === 'error') return ' is-error';
    return '';
  }
  function markDirty(kind, button) {
    if (getSaveState(kind) === 'saving') return;
    setSaveState(kind, 'dirty');
    if (button) {
      button.textContent = 'Guardar cambios';
      button.classList.remove('is-saved', 'is-warning', 'is-error');
    }
  }
  ...
  function ensureDocumentTabs() {
    ...
    const definitions = [['cover', 'Portada'], ['header', 'Cabecera'], ['sections', 'Secciones']];
  ...
  function responsibleBlock(report) {
    ...
    return rows.length
      ...
      : '<div class="document-preview-empty">Los responsables se toman de Información.</div>';
  }

  function bindDirtyForm(form, kind, button) {
    form?.querySelectorAll('input,select,textarea').forEach(control => {
      control.addEventListener('input', () => markDirty(kind, button));
      control.addEventListener('change', () => markDirty(kind, button));
    });
  }
  ...
  function renderCoverTab() {
    ...
    const config = configFor(report), cover = config.cover;
    host.innerHTML = `
      <section class="panel document-config-panel">
        <div class="panel-head"><div><h2>Portada</h2><p>Primera página del documento.</p></div></div>
        <form id="cover-config-form" class="document-config-grid">
          ...
          <div class="document-checks document-wide">
            ...
            <label class="check"><input type="checkbox" name="show_period" ${checked(cover.show_period)}> Período</label>
            <label class="check"><input type="checkbox" name="show_code" ${checked(cover.show_code)}> Código</label>
            <label class="check"><input type="checkbox" name="show_date" ${checked(cover.show_date)}> Fecha</label>
            <label class="check"><input type="checkbox" name="show_responsibles" ${checked(cover.show_responsibles)}> Responsables</label>
          </div>
          <div class="form-actions document-wide"><button class="button primary document-save-button${buttonClass('cover')}" type="submit" ${getSaveState('cover') === 'saving' ? 'disabled' : ''}>${buttonText('cover', 'Guardar portada')}</button></div>
        </form>
      </section>
      <section class="panel document-preview-panel">
        <div class="panel-head"><div><h2>Vista previa</h2><p>Primera página.</p></div></div>
        <article class="cover-preview ${cover.enabled ? '' : 'is-disabled'}">
          ...
          <div class="cover-preview-meta">
            ...
            ${cover.show_date ? `<span>${esc(report.elaboration_date || 'Fecha pendiente')}</span>` : ''}
          </div>
          ${cover.show_responsibles ? `<div class="cover-preview-responsibles">${responsibleBlock(report)}</div>` : ''}
        </article>
      </section>`;
    const form = host.querySelector('#cover-config-form');
    const button = form?.querySelector('.document-save-button');
    bindDirtyForm(form, 'cover', button);
    form?.addEventListener('submit', async event => {
      ...
      await saveDocumentConfig(next, 'cover');
    });
  }
  ...
  function renderHeaderTab() {
    ...
    const report = window.state?.activeReport;
    if (!host || !report) return;
    const config = configFor(report), header = config.header;
    host.innerHTML = `
      <section class="panel document-config-panel">
        <div class="panel-head"><div><h2>Cabecera</h2><p>Encabezado de las páginas internas.</p></div></div>
        <form id="header-config-form" class="document-config-grid">
          ...
          <div class="document-checks document-wide">
            ...
            <label class="check"><input type="checkbox" name="show_code" ${checked(header.show_code)}> Código</label>
            <label class="check"><input type="checkbox" name="show_version" ${checked(header.show_version)}> Versión</label>
            <label class="check"><input type="checkbox" name="show_period" ${checked(header.show_period)}> Período</label>
            <label class="check"><input type="checkbox" name="exclude_cover" ${checked(header.exclude_cover)}> Excluir de portada</label>
          </div>
          <div class="form-actions document-wide"><button class="button primary document-save-button${buttonClass('header')}" type="submit" ${getSaveState('header') === 'saving' ? 'disabled' : ''}>${buttonText('header', 'Guardar cabecera')}</button></div>
        </form>
      </section>
      <section class="panel document-preview-panel">
        <div class="panel-head"><div><h2>Vista previa</h2><p>Cabecera de páginas internas.</p></div></div>
    ...
    const form = host.querySelector('#header-config-form');
    const button = form?.querySelector('.document-save-button');
    bindDirtyForm(form, 'header', button);
    form?.addEventListener('submit', async event => {
      event.preventDefault();
      const next = configFor(window.state.activeReport);
      next.header = {
      ...
      await saveDocumentConfig(next, 'header');
    });
  }
  ...
  function renderSectionsTab() {
    ...
    host.innerHTML = `
      <section class="panel document-sections-panel">
        <div class="panel-head"><div><h2>Estructura</h2><p>Orden, título y visibilidad del PDF. Portada y Cabecera se administran aparte.</p></div></div>
        ...
        <div class="form-actions"><button class="button primary document-save-button${buttonClass('sections')}" type="button" id="save-document-sections" ${getSaveState('sections') === 'saving' ? 'disabled' : ''}>${buttonText('sections', 'Guardar estructura')}</button></div>
      </section>`;
    const list = host.querySelector('#document-section-list');
    const saveButton = host.querySelector('#save-document-sections');
    list?.addEventListener('input', () => markDirty('sections', saveButton));
    list?.addEventListener('change', () => markDirty('sections', saveButton));
    ...
    saveButton?.addEventListener('click', async () => {
      ...
      await saveDocumentConfig(next, 'sections');
    });
  }

  async function saveDocumentConfig(config, kind) {
    const report = window.state?.activeReport;
    if (!report?.id) return false;
    setSaveState(kind, 'saving');
    if (kind === 'cover') renderCoverTab();
    if (kind === 'header') renderHeaderTab();
    if (kind === 'sections') renderSectionsTab();
    try {
      ...
      window.state.activeReport = result?.report ? { ...result.report, document_config: config, document_config_json: JSON.stringify(config) } : { ...report, document_config: config, document_config_json: JSON.stringify(config) };
      const shared = result?.document_config_shared !== false;
      setSaveState(kind, shared ? 'saved' : 'local');
      if (typeof window.toast === 'function') {
        window.toast(shared
          ? 'Guardado y sincronizado.'
          : `Guardado solo en este navegador. No se pudo sincronizar con Google Sheets${result?.sharing_warning ? `: ${result.sharing_warning}` : '.'}`,
        !shared);
      }
      renderCoverTab();
      renderHeaderTab();
  ...
  window.InformtitDocumentWorkspace = Object.freeze({ configFor, version: '2.0.0' });
