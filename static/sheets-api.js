  ...
  async function postMany(items, options = {}) {
    ...
    async function worker() {
      while (true) {
        ...
        try {
          const result = await post(item.action, item.data, { attempts: 2 });
          results[index] = { ok: true, result, item };
        } catch (error) {
          results[index] = { ok: false, error: error.message || String(error), item };
        }
        completed += 1;
        if (typeof options.onProgress === 'function') {
          options.onProgress(completed, queue.length, results[index]);
        }
      }
    }
